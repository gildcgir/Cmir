...
function getClientId() {
  let id = sessionStorage.getItem("smir_client_id");
  if (!id) {
    id = globalThis.crypto?.randomUUID?.() || `c-${Date.now()}`;
    sessionStorage.setItem("smir_client_id", id);
  }
  return id;
}
...
async function loadPois() {
  ...
  try {
    ...
    if (!pois.length) {
      showMapStatus(
        "Нет мест на карте. Запустите lab: cd smir && bash scripts/start-lab.sh",
        true,
      );
    } else {
      showMapStatus("");
    ...
    showMapStatus(
      `Сервер недоступен (${API}). Запустите: cd smir && bash scripts/start-lab.sh`,
      true,
    );
  }
}
