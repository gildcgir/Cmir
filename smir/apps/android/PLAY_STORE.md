# Smir Android — публикация в Google Play

## Что это
Нативный Android shell (`com.smir.app`) с WebView: карта, киоск согласия (камера), аккаунт.
UI/логика — из веб-контура Smir; приложение даёт доступ к камере и deep links.

## Перед загрузкой в Play Console
1. Замените `smir_web_base` в `app/src/main/res/values/strings.xml` на production HTTPS URL.
...
# временно smir_web_base = http://10.0.2.2:3000/
```
