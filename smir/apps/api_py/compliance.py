...
def _fernet():
    ...
    try:
        ...
        return None
    key = os.environ.get("SMIR_DATA_KEY", "")
    if not key:
...
def default_legal_text(doc_type: str) -> str:
    ...
    return (
        ...
        "и срока хранения. Оператор: Smir Platform. Территория обработки: Грузия.\n\n"
...
def blockchain_record(conn, user_id: str, payload: dict) -> dict:
    ...
    return {"record_id": rid, "tx_hash": tx_hash, "created_at": t, "network": "smir-ledger-stub"}
