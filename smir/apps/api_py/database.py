"""SQLite persistence for Smir core (users, sessions, consents, wallets, POI)."""
...
DEFAULT_DB = Path(__file__).resolve().parent / "data" / "smir_test.db"


def app_env() -> str:
    return os.environ.get("SMIR_ENV", "test").lower()


def db_path() -> Path:
    explicit = os.environ.get("SMIR_DB_PATH", "")
    if explicit:
        return Path(explicit)
    base = Path(__file__).resolve().parent / "data"
    if app_env() == "prod":
        return base / "smir_prod.db"
    return base / "smir_test.db"
