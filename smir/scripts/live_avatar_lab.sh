...
RTSP_IN="${SMIR_RTSP:-rtsp://127.0.0.1:8554/gopro_main}"
RTMP_OUT="${SMIR_RTMP_AVATAR:-rtmp://127.0.0.1:1935/gopro_avatar}"
HLS_AVATAR="${SMIR_HLS_AVATAR:-http://127.0.0.1:8888/gopro_avatar/index.m3u8}"
...
pkill -f "smir_face.worker.*gopro_avatar" 2>/dev/null || true
sleep 1

log "3/5 GoPro → RTMP gopro_main (фон)"
export GOPRO_FPS="${GOPRO_FPS:-30}"
bash "$ROOT/scripts/gopro_usb_publish.sh" > /tmp/smir_gopro_pub.log 2>&1 &
PUB_PID=$!
echo "  publish PID=$PUB_PID (лог: /tmp/smir_gopro_pub.log)"
...
for i in $(seq 1 25); do
  ...
  if [[ "$i" -eq 25 ]]; then
    ...
    tail -20 /tmp/smir_gopro_pub.log
...
python -m smir_face.worker \
  ...
  --mask eye-rect \
  --track-smooth 0.55 \
  > /tmp/smir_avatar_worker.log 2>&1 &
WORKER_PID=$!
echo "  worker PID=$WORKER_PID (лог: /tmp/smir_avatar_worker.log)"
...
echo ""
echo "  Логи: tail -f /tmp/smir_avatar_worker.log"
echo ""
