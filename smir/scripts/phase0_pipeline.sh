#!/usr/bin/env bash
# Smir Phase 0 — full video pipeline (order: sample → process → serve)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
API_URL="${SMIR_API_URL:-http://localhost:8090}"
...
(
  ...
  python -m smir_face.worker --input "$DEMO_IN" --output "$DEMO_OUT" --demo-fallback
)
...
api = "$API_URL"
poi_id = "$POI_ID"
email, password = "phase0@smir.test", "testpass123"
try:
