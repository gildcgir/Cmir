#!/usr/bin/env bash
# Smir — поднять полную lab-среду (ingest + API + web + health poll)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PID_DIR="$ROOT/.lab-pids"
mkdir -p "$PID_DIR"

echo "==> Smir lab start (SMIR_ENV=${SMIR_ENV:-test})"
export SMIR_ENV="${SMIR_ENV:-test}"
...
nohup env SMIR_ENV="${SMIR_ENV:-test}" python3 "$ROOT/apps/api_py/server.py" >"$PID_DIR/api.log" 2>&1 &
echo $! >"$PID_DIR/api.pid"
disown || true

sleep 1
echo "==> Demo POI (test DB)"
SMIR_ENV="${SMIR_ENV:-test}" python3 "$ROOT/scripts/seed_test_poi.py" || echo "WARN: seed_test_poi failed"
