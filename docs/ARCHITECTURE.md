...
Cmir агрегирует видеопотоки с камер в точках интереса (POI), обрабатывает лица по политике согласия, формирует описание сцены и показывает рейтинги на карте. Участники с согласием получают кошелёк и учёт времени в кадре для доли дохода; токены ST (ЦФА) и UT (лояльность) — отдельный контур.
...
Переиспользуем **паттерны** (consent API, JWT, ST/UT), не код 1:1. Cmir — отдельный продукт и репозиторий.
