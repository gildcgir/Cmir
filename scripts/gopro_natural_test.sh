#!/usr/bin/env bash
# Натуральный тест Cmir с GoPro USB (HERO13 Webcam)
set -euo pipefail
...
python -m cmir_face.worker --input "$RAW" --output "$MEDIA/gopro_avatar.mp4" \
...
poi_id = "$POI_ID"
raw = "$RAW"
email, password = "gopro@cmir.test", "testpass123"
try:
